...
/* ─────────────────────────────────────────
   ...
   Isolated from the app design system and shared site chrome
───────────────────────────────────────── */
...
function LandingHeader() {
  return (
    <nav className="lp-site-header" aria-label="Main navigation">
      <Link href="/" className="lp-site-logo-link" aria-label="Nyabag home">
        <Image
          ...
          height={118}
          priority
          className="lp-site-logo"
        />
      </Link>

      <div className="lp-site-header-links" role="list">
        {landingNavLinks.map((link) => (
          <Link key={link.href} href={link.href} className="lp-site-header-link" role="listitem">
            {link.label}
          </Link>
        ))}
      </div>

      <Link href="/#early-access" className="lp-site-header-cta">
        Join early access
      </Link>
    </nav>
  );
...
function LandingFooter() {
  return (
    <footer className="lp-site-footer" role="contentinfo">
      <div className="lp-site-footer-watermark" aria-hidden="true" />
      <div className="lp-site-footer-inner">
        ...
        <nav className="lp-site-footer-nav" aria-label="Footer links">
          <div className="lp-site-footer-col">
            <div className="lp-site-footer-col-title">Product</div>
            {landingNavLinks.map((link) => (
              <Link key={link.href} href={link.href} className="lp-site-footer-link">
                {link.label}
              </Link>
            ))}
          </div>
          <div className="lp-site-footer-col">
            <div className="lp-site-footer-col-title">Legal</div>
...
/* LANDING-ONLY SITE CHROME — intentionally not connected to app design-system tokens */
.lp-site-header{position:fixed;top:0;left:0;right:0;z-index:200;height:60px;display:flex;align-items:center;justify-content:space-between;padding:0 40px;border-bottom:1px solid rgba(255,255,255,.07);background:rgba(8,9,10,.82);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px)}
.lp-site-logo-link{display:flex;align-items:center;text-decoration:none}
.lp-site-logo{display:block;width:112px;height:auto}
.lp-site-header-links{display:flex;align-items:center;gap:28px}
.lp-site-header-link{color:rgba(232,232,232,.68);font-size:13.5px;text-decoration:none;transition:color .2s ease}
.lp-site-header-link:hover,.lp-site-header-link:focus-visible{color:#fff}
.lp-site-header-cta{display:inline-flex;align-items:center;justify-content:center;min-height:34px;padding:7px 18px;border-radius:6px;background:#fff;color:#08090a;font-size:13.5px;font-weight:500;text-decoration:none;transition:opacity .2s ease,transform .2s ease}
.lp-site-header-cta:hover,.lp-site-header-cta:focus-visible{opacity:.88;transform:translateY(-1px)}
.lp-site-footer{position:relative;overflow:hidden;border-top:1px solid rgba(255,255,255,.07);padding:60px 40px 100px;background:linear-gradient(180deg,rgba(255,255,255,.018),transparent 34%),#08090a;color:#e8e8e8}
.lp-site-footer::before{content:"";position:absolute;inset:0;background-image:linear-gradient(90deg,rgba(255,255,255,.022) 1px,transparent 1px),linear-gradient(180deg,rgba(74,158,255,.045),transparent 42%);background-size:82px 100%,100% 100%;opacity:.38;pointer-events:none}
.lp-site-footer-watermark{position:absolute;left:50%;bottom:-16px;width:min(980px,92vw);height:200px;transform:translateX(-50%);background:linear-gradient(95deg,rgba(255,255,255,.08),rgba(74,158,255,.16) 42%,rgba(255,255,255,.04) 72%,rgba(255,255,255,.075));-webkit-mask:url("/assets/Nyabag-Dark-Logo.svg") center / contain no-repeat;mask:url("/assets/Nyabag-Dark-Logo.svg") center / contain no-repeat;opacity:.70;pointer-events:none}
.lp-site-footer-inner{position:relative;z-index:1;max-width:1080px;margin:0 auto 86px;display:flex;justify-content:space-between;gap:40px;flex-wrap:wrap}
.lp-site-footer-brand{display:flex;flex-direction:column;align-items:flex-start;gap:26px;color:rgba(232,232,232,.62)}
.lp-site-footer-credit{display:inline-block;color:rgba(232,232,232,.58);font-size:12px;line-height:1.5}
.lp-site-footer-credit-link{color:rgba(232,232,232,.78);text-decoration:underline;text-decoration-color:rgba(232,232,232,.24);text-underline-offset:4px;transition:color .2s ease,text-decoration-color .2s ease}
.lp-site-footer-credit-link:hover,.lp-site-footer-credit-link:focus-visible{color:#4a9eff;text-decoration-color:#4a9eff}
.lp-site-footer-nav{display:flex;gap:48px;flex-wrap:wrap}
.lp-site-footer-col{display:flex;flex-direction:column;gap:10px}
.lp-site-footer-col-title{color:rgba(232,232,232,.44);font-size:11.5px;letter-spacing:.08em;text-transform:uppercase;margin-bottom:4px}
.lp-site-footer-link{color:rgba(232,232,232,.62);font-size:13px;text-decoration:none;transition:color .2s ease}
.lp-site-footer-link:hover,.lp-site-footer-link:focus-visible{color:#fff}
...
@media(max-width:900px){
  .lp-site-header{padding:0 24px}
  .lp-site-header-links{display:none}
  .lp-site-footer{padding:54px 24px 30px}
  .lp-site-footer-inner{flex-direction:column;gap:32px;margin-bottom:74px}
  .lp-site-footer-watermark{bottom:-38px;width:90vw;height:36vw}
}

@media(max-width:540px){
  .lp-site-header{height:58px;padding:0 16px}
  .lp-site-header-cta{padding:7px 12px;font-size:12.5px}
